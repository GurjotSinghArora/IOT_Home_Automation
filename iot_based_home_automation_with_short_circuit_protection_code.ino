...
#define FIREBASE_HOST "iot-automation-with-protection-default-rtdb.firebaseio.com"                     //Your Firebase Project URL goes here without "http:" , "\" and "/"
#define FIREBASE_AUTH "7FKVfwynHVqJmCvodMCA0qGL3oNfhbvvhl8xK4UH"       //Your Firebase Database Secret goes here
#define WIFI_SSID "iphone"                                               //your WiFi SSID for which yout NodeMCU connects
#define WIFI_PASSWORD "00000000"                                      //Password of your wifi network 
...
void loop() 
{
  ...
  Serial.println(ldrValue);                                                 // print out the value of ldr sensor
  delay(200);

  if(ldrValue<700)
     {Firebase.setString("MCB_Status","Tripped");
      digitalWrite(Relay1,HIGH);
      digitalWrite(Relay2,HIGH);                                            //it is to protect the system 
      digitalWrite(Relay3,HIGH);                                            //when MCB trips at the same time all switches are switched off
      digitalWrite(Relay4,HIGH);                                            // will reconnect server and remain in the loop to continue 
      firebasereconnect();
     }
      
  else
     {Firebase.setString("MCB_Status","Normal");
     }
  ...
  if(val1==1)                                                             // If, the Status is 1, turn on the Relay1
     {
      digitalWrite(Relay1,LOW);                                           // Low means to switch on the relay
